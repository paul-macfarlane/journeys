...
Next.js 16 App Router, React 19, TypeScript, pnpm. Drizzle ORM on Neon Postgres in deployed environments and on a docker-compose Postgres locally. better-auth with Google and Discord, both always enabled. Tailwind v4 with shadcn, zod 4, react-hook-form, next-themes. Vercel AI SDK for AI features. Vitest and Playwright. Deployed on Vercel: Production from `main`, and a long-lived `staging` branch with a stable domain. There are no pull-request preview deployments: Vercel's Ignored Build Step skips every branch except `staging` and `main`. The Vercel Preview environment, which the `staging` build uses, carries a full set of env values (`human-prerequisites.md` §9) so that build passes env validation; its OAuth callbacks and `BETTER_AUTH_URL` point at the staging domain, so sign-in works on staging and production. Project layout, lint, format, and hook conventions mirror `paul-macfarlane/paulitakes`. The canvas uses React Flow (`@xyflow/react`, MIT) with `@dagrejs/dagre` auto-layout (the maintained fork, not the unmaintained `dagre` package). Rich text uses Tiptap.
...
Authors authenticate via better-auth. Any signed-in user can create a Project and becomes its first Member. A Member adds another by the email of an existing account; there are no invitation emails or pending states. Draft edits are last-write-wins with no locking. An Author edits their display name, bio, links, and the Author page switch on one Settings page (amended 2026-09-23 by ticket 52; see the `[SCOPE CHANGE]` below).
...
Withdrawn — see the `[SCOPE CHANGE] 2026-09-26` record below (ticket 14, `wontfix`); this section's code never merged. The only AI consumer today is the Judge (deciding Prompts; ticket 80 may add AI editing); see the "**AI authoring is cut.**" bullet in that record.
...
### [SCOPE CHANGE] 2026-09-23 — free text decides the Choice (ticket 43)

Amends "Graph document" and "Public URL and runner": a Prompt gains `decides: boolean` (zod default `false`, so every stored Draft and Published Version parses unchanged). A deciding Prompt is required, and only a Step with two or more Choices offers it. In the runner, such a Step shows its textbox and one "Continue" button instead of its Choices; the Response is posted on its own and judged by jev (`typesafe-ai/jev` on the Vercel AI Gateway, `experimental_evaluate`, a `choice` question over the Step's Choice labels). At a probability of 0.5 or more the Run advances along the judged Choice exactly as if its button had been pressed; below that, or with no key, a failed or rate-limited call (one decision per Run per second), or an answer naming no Choice of the Step, the Choices are shown under "Choose for yourself" with the judge's pick, when it had one, marked. Preview asks the judge the same way, never advances on the answer, and shows the pick and its probability; publishing a deciding Prompt with no gateway key succeeds with a warning. What stays: Runs are unchanged — a Run records the Choice taken and the Response, as for a pressed Choice; Published Versions are never rewritten; the judge sees only the Step (its title, plain-text content, Prompt label, Choice ids and labels, and the Response) — never an Ending, an Outcome, another Step, or another Participant's Response; the runner stays JavaScript-free. Reason: Paul's staging regression notes of 2026-09-22, item 11, grilled the same day (Q15, Q18, Q20, Q21). Recorded in ticket 43.

### [SCOPE CHANGE] 2026-09-23 — Author pages and the display name (ticket 52)

Amends "Public pages and discovery", "Members and auth", and the Out of Scope line "public discovery, search, a directory, or a marketplace (discovery is link-only)": an Author may turn on a public Author page at `/authors/{user-id}` (the id is the address, as for Projects and Journeys; no handle). It is off by default and a 404 until the Author turns it on, so a name that Google or Discord supplied is never published without the Author's own switch. When on, it renders in the app's own Theme inside the runner frame: the provider avatar, the display name, a plain-text bio (at most 1,000 characters, line breaks kept), a row of links (at most one each of LinkedIn, GitHub, Instagram, Facebook, and a website; absolute `https:` URLs, the four platform kinds pinned to their platform's host), and the Projects the Author is a Member of that have at least one live Journey, newest activity first, each linking to `/p/{project-id}` — or "No published journeys yet." The public Project page names, under its title, the Members whose pages are on, each a link, and nothing otherwise. Discovery stays link-only: there is no index of Authors and the landing page still lists nothing; an Author page is the one place that browses across Projects, and only its own Author's. An Author also edits their display name (1–60 characters, trimmed, never blank; better-auth does not overwrite it on later sign-ins) on a Settings page under `/projects`, reached from a "Settings" row in the user menu, which also holds the bio, the links, and the Author page switch; fields save on blur as Project settings do, the switch on change. The avatar stays the provider's; images remain URL-only and nothing is uploaded. Schema: `user.bio`, `user.links`, and `user.public` (default false), one additive migration. The privacy page gains one sentence saying what turning the page on makes public. Reason: Paul's staging regression notes of 2026-09-23, item 4, grilled the same day in its own thread. Recorded in ticket 52.

### [SCOPE CHANGE] 2026-09-26 — post-hackathon: polish, stability, no AI authoring

The hackathon ended 2026-09-25. Paul's direction for what follows: a polished product built on bug fixes, stability, and a clean technical design, with no feature bloat. Grilled on 2026-09-26.

- **AI authoring is cut.** Ticket 14 is `wontfix`; no generated Drafts. AI stays in two places only: the Judge, which reads a Response and never writes, and AI editing (ticket 80, a low-priority reach goal). AI editing proofreads and tightens a Step's content, never adds content, and shows each change for the Author to accept or reject before it applies.
- **Completion** (amends ticket 10's analytics and ADR-0002): a Run is a Completion once it has reached an Ending, and backtracking does not undo it. Its Outcome is that of the latest Ending it reached (ticket 75).
- **The Judge may find a Response unclear.** When it does, the Participant chooses, as when the Judge has no answer (ticket 76).
- **Concurrent edits:** a stale save of a Draft or of Project and Journey settings is refused with a reload prompt, never silently overwritten (ticket 73).
- **Account deletion** is added (ticket 77). Projects where the Author is the only Member are deleted with their contents. Shared Projects stay with their other Members.
- **Withdrawn:** manual layout (17), drag-and-drop Journey ordering (41). **Parked:** rename (44), loading states (39, blocked upstream), Atlas refresh (45).
- **Order:** 71 → 72 (codebase design review) → 73 → 74 → 75 → 76 → 77 → 42 → 78 → 79 → 53 → 57 → 80. Ticket 72's findings may add tickets from 81 and reshape anything after it.
