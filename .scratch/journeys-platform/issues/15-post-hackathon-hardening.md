...
- [ ] **Rich-text contract gaps.** `hardBreak` (Shift+Enter) is outside the allowed set and is stripped at save; decide whether to admit it. An image nested in a list item is dropped at save whatever its attrs (review finding F9, ticket 03; since ticket 30 nothing about an image refuses a write, so the old refuse-versus-drop asymmetry is gone).
