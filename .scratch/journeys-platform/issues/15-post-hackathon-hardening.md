...
- [ ] **Rich-text contract gaps.** `hardBreak` (Shift+Enter) is admitted by ticket 40 (Paul, 2026-09-22); this bullet closes when 40 lands. An image nested in a list item is dropped at save whatever its attrs (review finding F9, ticket 03; since ticket 30 nothing about an image refuses a write, so the old refuse-versus-drop asymmetry is gone).
