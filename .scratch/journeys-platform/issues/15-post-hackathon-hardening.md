...
- [ ] **Rich-text contract gaps.** `hardBreak` (Shift+Enter) is outside the allowed set and is stripped at save; decide whether to admit it. An uncredited image nested in a list item refuses the write although a credited one is dropped (review finding F9, ticket 03).
