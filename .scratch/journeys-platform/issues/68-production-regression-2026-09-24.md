...
1. **pre-hackathon** (data, no code) — the seed Project's link preview, `/p/00000000-5eed-4000-8000-000000000001` — `og:description` is empty and there is no `<meta name="description">`, so a judge pasting the Play link into Slack, Discord, or iMessage sees "Journey Stories" with no line under it; the three cases carry descriptions but the Project itself has none (the seed writes Journey descriptions only, `scripts/seed/journey-stories-seed.ts`). Expected: a sentence such as "Three interactive cases for trauma-informed healthcare education." — **Paul**: type a description into the seed Project's Settings on production (the agent does not edit seed content). Post-hackathon: fall back to the site description when a Project's is blank, so no Project ever previews empty.
