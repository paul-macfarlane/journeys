...
- **After the chain:** `origin/staging` at `2fe0da5` (#70 and #73 merged) was merged into the branch as `b405c3c`. The only conflicts were the two per-ticket DoD captures, resolved to this ticket's versions; the rest merged clean and touches nothing the seed documents depend on, so the CI run on the merged head is the proof for that state.
