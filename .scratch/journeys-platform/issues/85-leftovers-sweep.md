...
### [PROGRESS] 2026-09-26: drop-migration follow-up

Paul promoted chunk 2 to production (PR #104, merged 22:31 UTC; Migrate green on `main`), so nothing live references `project.description` any more. `drizzle/0012_drop_project_description.sql` is exactly `ALTER TABLE "project" DROP COLUMN "description";`, generated by `pnpm db:generate`. The full chain passed with it applied (e2e 121/121), and the column is gone from the e2e database: `test-results/85-drop-migration-commands.txt`. Tickets that run `db:generate` after this merges no longer see a stray drop.
