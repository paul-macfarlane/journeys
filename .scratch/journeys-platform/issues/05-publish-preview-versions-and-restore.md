...
### [CLOSEOUT] 2026-09-20 — addendum after the PR #15 rework

Same PR, same branch. Rework commits: `ca5efb4` (Tiptap rendering, Publish disabled with nothing to publish, `PreviewFrame`, own-property lookups in the validator; orchestrator), `08adbc2` (recaptured evidence and records), plus Paul's own commit of `pnpm-lock.yaml`. Verified run command all exit 0 at `ca5efb4` (111 unit tests, 19 e2e specs); the criterion table above stands with the evidence recaptured, and AC-5's Preview screenshot now shows Tiptap-rendered content. The renderer question is closed (Tiptap adopted); the `CLAUDE.md` lockfile line now states the real constraint (the Atlas plugin's commit-time secret scrub) rather than a bare rule. Human follow-ups now: (1) commit the lockfile and merge PR #15; (2) decide whether to give the Atlas plugin's secret-scrub hook a lockfile allowlist so agents can commit `pnpm-lock.yaml` themselves.

### [SCOPE CHANGE] 2026-09-20 — approved by Paul (PR #15 review, second round)

- **A Published Version carries the Journey's title and description** as they read at publish time (migration 0004 adds `published_version.title` and `.description`, defaulted and backfilled from the Journey for existing rows). Editing either counts as an unpublished change, so Publish is available again after a description edit, and a rename never changes what is live until the next publish. Ticket 06's runner and ticket 07's public Project page read the version's title and description, not the Journey row's.
- **The "Draft" heading on the Journey page reads "Steps"**, and the publish hint says "Publish to make what is here live." / "Everything here is live." — the page shows the Journey's content; the controls say whether it is live.

### [PROGRESS] 2026-09-20 — second rework after Paul's PR #15 review

Applied by the orchestrator in `6050798`: migration `0004_massive_khan` adds `published_version.title` and `.description` (defaulted, backfilled from the Journey); `publishDraft` snapshots both; `getLiveVersion` returns them with the document, and the Journey page treats a title or description edit as an unpublished change. The Journey page heading "Draft" reads "Steps"; the publish hint reads "Publish to make what is here live." / "Everything here is live.". `publish-versions-and-restore` now edits the description through the Edit dialog after version 1, sees Publish re-enabled, and checks that version 2's row carries the new description while version 1's does not. Verified run command all exit 0 at `6050798`: 111 unit tests, 19 e2e specs; evidence recaptured, `dod-3-migration.txt` now covers 0003 and 0004. Ticket 06 carries a note to read title and description from the pinned version.
