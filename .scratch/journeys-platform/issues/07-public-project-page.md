...
**What to build:** A Participant opens `/p/{project-slug}` and sees the Project's title, its rich-text description, and its currently published Journeys (title, description, link); unpublished and never-published Journeys are absent. Authors edit the Project description with the same Tiptap editor and allowed node set as Steps. Discovery is link-only: no index, search, or cross-project browsing anywhere.
