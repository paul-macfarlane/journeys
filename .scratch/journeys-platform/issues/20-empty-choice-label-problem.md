...
- [x] Seam A: a document with one whitespace-only Choice label yields exactly one `empty-choice-label` problem with that Step's and Choice's ids; the three seeded documents still yield none.
- [x] Seam B: dragging a Choice on the canvas shows the problem on the arrow, under the Choice row, and in the header count; typing a label clears all three; publishing while a label is empty is refused with the message in the Validation list.
- [x] Spec `[SCOPE CHANGE]` recorded; README's publish sentence mentions labels if it lists the rules.
...
**PR:** https://github.com/paul-macfarlane/journeys/pull/39 (`feat/20-empty-choice-label` → `staging`). Per the tracker rule Paul set on 2026-09-21, this closeout commit carries `Status: done`; merging the PR is the acceptance that lands it on `staging`. Status log: `ready-for-agent` → `in-progress` → `ai-review` → `done`.

**Repository delivery:** `journeys`, base `staging` at `eb0c258`, direct checkout, no worktrees, five commits: `12cc8eb` claim and execution plan, `db62b63` the rule, its tests, the e2e spec, and the spec amendment, `c3f2793` review fixes, `868ecae` evidence and the review record, and this closeout. Structure as planned: one session, three seams in order, no delegation.

**Exact verified run command** (local; docker Postgres on 5436; captured over the `c3f2793` tree, committed in `868ecae` as `test-results/dod-1-commands.txt`):

```
pnpm lint && pnpm format:check && pnpm typecheck && pnpm test && pnpm build && E2E_EVIDENCE=canvas-empty-choice-label pnpm test:e2e
```

Every command exits 0; Vitest 326 passed in 21 files; Playwright 81 passed (production build on port 3100, `retries` 0, nothing flaky). No `db:migrate` (no schema change), no dependency or lockfile change, no deployed-target check (Paul's 2026-09-20 decision).

| Criterion | Verdict | Evidence |
|---|---|---|
| AC-A Seam A: one whitespace-only label yields exactly one `empty-choice-label` problem with the Step's and Choice's ids; the seeded documents yield none | PASS | `test-results/ac-1-empty-choice-label-unit.txt` (the three new cases and the three "is publishable" cases named) |
| AC-B Seam B: the problem on the arrow, under the Choice row, and in the header count; typing clears all three; publishing with an empty label refused with the message listed | PASS | `test-results/canvas-empty-choice-label/canvas-empty-choice-label.png` and `-refused.png`; the spec's assertions in `e2e/canvas.spec.ts` |
| AC-C spec `[SCOPE CHANGE]` recorded; README | PASS | `.scratch/journeys-platform/spec.md` (rule list, stories 33 and 37, testing bullet, `[SCOPE CHANGE]` naming ticket 20); README lists no rules, so it is unchanged |
| DoD-1 command chain green | PASS | `test-results/dod-1-commands.txt` |
| DoD-2 PASS artifacts committed, fixture journeys only | PASS | `868ecae`; the e2e Draft is built in the test, no participant data |

**Deviations:** none. **Red team:** not run, as the execution plan records. **Follow-ups:** none.
