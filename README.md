...
- `/projects/<project-id>/journeys/<journey-id>` — one journey: edit its title
  ...
  outcome if you want endings grouped (searchable too; an outcome the last
  ending drops is still removed with it), and run validation on demand. The draft is
...
- `/j/<journey-id>/<step-id>` — one step of a run: its text, its choices as
  ...
  records its outcome, if it has one; starting over begins a new run and leaves the old one
